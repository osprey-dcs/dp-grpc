...
The two selectors treat an empty criteria list oppositely, and the difference is deliberate.  A ConfigurationSelector that is present with an empty criteria list is rejected with an ExceptionalResult; omit the ConfigurationSelector entirely to query the full TimeRange unconditionally.  It is an optional restriction, so matching everything would be a no-op identical to omitting it, and rejecting keeps a half-built selector from silently returning an empty result.  A PvSelector metadata query with an empty criteria list matches all PVs in the archive, since the PvSelector is required and its criteria must mean something; prefer stating an all-PV query explicitly as a name regex pattern of ".*".
